...
- The MUI dense experiment lives under [`src/mui-dense/`](./src/mui-dense/) and uses `@mui/material`, `@mui/x-data-grid-pro`, `@mui/x-tree-view`, and `@mui/x-tree-view-pro` with both a gallery controls panel and a separate advanced dense-overrides panel. The local reusable entrypoint now lives under [`src/mui-dense/lib/`](./src/mui-dense/lib/) and exposes the app-facing dense surface the gallery is meant to validate: `DEFAULT_DENSE_SETTINGS`, a flat `DenseSettings` shape, the top-level `createDenseTheme(denseSettings, palette)` builder, and a layered Data Grid API built around `DenseDataGrid` plus pure sizing helpers. `DenseDataGrid` now measures its own default text probes unless a caller supplies explicit metrics, while still allowing caller overrides for the representative row/header probe content when needed. Gallery-only presets, control copy, and adapter glue now live beside the page in files such as [`src/mui-dense/gallery-dense.ts`](./src/mui-dense/gallery-dense.ts), while the gallery shell and demo workspace stay in the local `gallery-*.tsx` files. On wider screens, the control surface lives in its own scrollable side panel and the component gallery scrolls in a separate adjacent pane while keeping its wide-screen reflow behavior.
