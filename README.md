...
- The MUI dense experiment lives under [`src/mui-dense/`](./src/mui-dense/) and uses `@mui/material`, `@mui/x-data-grid-pro`, `@mui/x-tree-view`, and `@mui/x-tree-view-pro` with both a gallery controls panel and a separate advanced theme-features panel. The local dense-library entrypoint now lives under [`src/mui-dense/dense/`](./src/mui-dense/dense/) and exposes the reusable pieces the gallery is meant to validate: presets, the top-level `createDenseTheme` builder, and the `DenseDataGrid` wrapper that owns row/header measurement plus block-padding tuning. Gallery-only control copy and adapter glue now live beside the page in files such as [`src/mui-dense/gallery-density.ts`](./src/mui-dense/gallery-density.ts), while the gallery shell and demo workspace stay in the local `gallery-*.tsx` files. On wider screens, the control surface lives in its own scrollable side panel and the component gallery scrolls in a separate adjacent pane while keeping its wide-screen reflow behavior.
