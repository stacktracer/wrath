...
- The MUI dense experiment lives under [`src/mui-dense/`](./src/mui-dense/) and uses `@mui/material`, `@mui/x-data-grid-pro`, `@mui/x-tree-view`, and `@mui/x-tree-view-pro` with both a gallery controls panel and a separate advanced dense-overrides panel. The local reusable entrypoint now lives under [`src/mui-dense/lib/`](./src/mui-dense/lib/) and exposes the app-facing dense surface the gallery is meant to validate: `DEFAULT_DENSE_SETTINGS`, a flat `DenseSettings` shape, the top-level `createDenseTheme(denseSettings, palette)` builder, and the `DenseDataGrid` wrapper that owns density-compensated row/header measurement plus CSS-length content-vertical-padding tuning while leaving ordinary header-filter props on `DataGridPro`. Gallery-only presets, control copy, and adapter glue now live beside the page in files such as [`src/mui-dense/gallery-dense.ts`](./src/mui-dense/gallery-dense.ts), while the gallery shell and demo workspace stay in the local `gallery-*.tsx` files. On wider screens, the control surface lives in its own scrollable side panel and the component gallery scrolls in a separate adjacent pane while keeping its wide-screen reflow behavior.
