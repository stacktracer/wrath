...
The desired end state for the experiment itself is a gallery app that is still easy to iterate on, but whose moving parts are understandable. `App` should mostly compose already-defined pieces instead of being the place where all policy is invented. A good target for the first pass is entirely local to `src/mui-dense/`: one module for density state and presets, one for theme derivation, one for advanced overrides, one for MUI X measurement/hooks, one for gallery shell components, and one or more small modules that declare the actual demo sections. The browser test should stay, but the tricky logic should also have direct tests: presets, animation policy, and auto-height calculation should be testable without mounting the whole gallery. The point of this pass is to make the experiment legible and modular before deciding what deserves to move into a cross-experiment reusable directory.
...
If wrappers are introduced, they should be justified by the amount of shared policy they carry. In some areas that may mean only a few targeted wrappers; in others it may mean a broader wrapper set if that turns out to be the cleanest way to keep shared `sx` and default prop policy from drifting. That can include a dense Data Grid that owns its measurement and header/body alignment policy, and it can also include thin wrappers around common MUI inputs. The Data Grid sizing logic should be treated as a strong candidate for the densification API rather than as gallery-only glue: applications are likely to need the same behavior, or a parameterized version of it with default tuning. The first pass should therefore extract that logic behind an explicit local API that already looks like something a real app could call, even if we later discover that some part of the measurement path is too coupled and needs another design pass. The key constraint is that those wrappers should stay thin, preserve the caller's access to the underlying MUI component surface, and avoid becoming opaque substitute components with their own parallel API.
